...
class PackageModel extends PackageEntity {
  ...
  static String _resolveProviderDisplayName({
    required String? rawProviderName,
    required String packageTitle,
  }) {
    final providerName = (rawProviderName ?? '').trim();
    final normalizedProvider = providerName.toLowerCase();
    final isGenericProvider =
        normalizedProvider == 'egypt travel' ||
        normalizedProvider == 'egypttravel';

    if (providerName.isNotEmpty && !isGenericProvider) {
      return providerName;
    }

    final title = packageTitle.trim();
    if (title.isNotEmpty) {
      return title;
    }
  ...
  factory PackageModel.fromJson(Map<String, dynamic> json) {
    ...
    final rawLocationLink = json['location_link']?.toString().trim();
    final normalizedLocationLink =
